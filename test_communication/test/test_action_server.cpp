...
template <typename ActionT>
typename rclcpp_action::Server<ActionT>::SharedPtr receive_goals(
    rclcpp::Node::SharedPtr node,
    const std::string& action_type_name,
    const std::vector<ExpectedGoalRequest<ActionT>>& expected_goal_requests) {
  auto logger = node->get_logger();

  auto goal_callback = [expected_goal_requests, logger](auto /* goal_id */, auto goal) {
  ...
  auto cancel_callback = [logger](auto /* goal_handle */) {
    RCLCPP_ERROR(logger, "received unexpected request to cancel goal");
    return rclcpp_action::CancelResponse::ACCEPT;
  };

  auto execute_callback = [expected_goal_requests, logger](auto goal_handle) {
  ...
  return rclcpp_action::create_server<ActionT>(node, std::string("test/action/") + action_type_name, goal_callback,
                                               cancel_callback, execute_callback);
}

std::vector<ExpectedGoalRequest<test_msgs::action::Fibonacci>> generate_expected_fibonacci_goals(
...

  {
    ...
    expected_goal.goal_is_expected = [](auto goal) {
    ...
    expected_goal.goal_is_valid = [](auto) { return true; };

    expected_goal.execute_goal = [logger](auto goal_handle) {
...
std::vector<ExpectedGoalRequest<test_msgs::action::NestedMessage>> generate_expected_nested_message_goals(
    rclcpp::Logger logger) {
...
int main(int argc, char** argv) {
  ...
  if (action == "Fibonacci") {
    server = receive_goals<test_msgs::action::Fibonacci>(node, action,
                                                         generate_expected_fibonacci_goals(node->get_logger()));
